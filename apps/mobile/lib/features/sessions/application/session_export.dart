...
Future<Uint8List> buildSessionPdf({
  ...
  pw.Widget row(String left, String right, {bool bold = false}) => pw.Padding(
  ...
  document.addPage(
    pw.MultiPage(
      ...
      build: (context) => [
        ...
        pw.Text(
          'Total: ${formatMoney(detail.summary.grandTotal)}',
...
Future<Uint8List> buildSummaryImage({
  ...
  canvas.drawRect(
    ...
    ui.Paint()..color = const ui.Color(0xFFFAF9F7),
  );
  canvas.drawRRect(
    ui.RRect.fromRectAndRadius(
      const ui.Rect.fromLTWH(0, 0, width, 260),
  ...
  text(Brand.appName, 64, 48, size: 32, color: const ui.Color(0xB3FFFFFF));
  text(
    detail.summary.name,
    64,
    96,
  ...
  for (final p in participants) {
    ...
    final amount = '${net.cents > 0 ? '+' : ''}${formatMoney(net)}';
  ...
  text(
    'Abre el enlace para marcar lo tuyo',
    64,
    height - 84,
    size: 30,
