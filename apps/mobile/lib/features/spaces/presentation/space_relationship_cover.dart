...
/// A bilateral cover uses the other identity as its heading.  It intentionally
/// has none of the group vocabulary or member administration.
class RelationshipSpaceCover extends ConsumerStatefulWidget {
  const RelationshipSpaceCover({super.key, required this.space});

  final Space space;

  @override
  ConsumerState<RelationshipSpaceCover> createState() =>
      _RelationshipSpaceCoverState();
}

class _RelationshipSpaceCoverState
    extends ConsumerState<RelationshipSpaceCover> {
  var _scanning = false;
        ...
        children: [
          ...
          if (!peopleError && !contextReady && !countingPeople) ...[
            EmptyState(
              ...
              title: l10n.relationshipNeedsAcceptance,
              body: l10n.relationshipNeedsAcceptanceBody,
            ),
            const SectionGap(),
          ],
          CoverSection(
            title: l10n.spaceCoverRelationshipBalance,
            action: TextButton(
              onPressed: () =>
                  context.push('/home/spaces/${space.id}/balances'),
              child: Text(l10n.spaceCoverViewBalance),
            ),
