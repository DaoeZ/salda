...
void main() {
  group('BalanceEngine', () {
    test('sin tickets: todo a cero y sin liquidaciones', () {
      final result =
          BalanceEngine.compute(participantIds: ['a', 'b'], tickets: []);
    ...
    test('liquidación congelada con participante desconocido es error', () {
      expect(
        ...
        throwsA(isA<DomainException>()
            .having((e) => e.code, 'code', DomainException.unknownParticipant)),
      );
    });
    ...
    test(
        'propiedad: aplicar las liquidaciones sugeridas deja a todos en paz '
        '(300 sesiones sembradas)', () {
      final rng = Random(99);
      for (var s = 0; s < 300; s++) {
        final n = 2 + rng.nextInt(5);
        final pids = List<String>.generate(n, (i) => 'p$i');
        final tickets = List<TicketContribution>.generate(
          1 + rng.nextInt(5),
          (_) {
            ...
            return TicketContribution(
              ...
              grandTotal: total,
              consumption: {
                for (var i = 0; i < n; i++) pids[i]: shares[i],
              },
            );
          },
        );
        final result =
            BalanceEngine.compute(participantIds: pids, tickets: tickets);

        // Invariante 1: Σ outstanding == 0.
        final sum = result.balances.values
            .fold<int>(0, (a, b) => a + b.outstanding.cents);
        ...
        for (final settlement in result.settlements) {
          ...
          after[settlement.from] = after[settlement.from]! + settlement.amount.cents;
          after[settlement.to] = after[settlement.to]! - settlement.amount.cents;
        }
        for (final p in pids) {
    ...
    test('determinismo: mismo input, mismo output (incluido orden)', () {
      ...
      final r1 =
          BalanceEngine.compute(participantIds: ['a', 'b', 'c'], tickets: tickets);
      final r2 =
          BalanceEngine.compute(participantIds: ['a', 'b', 'c'], tickets: tickets);
